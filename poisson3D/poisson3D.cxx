...
// <h1>Introduction Example 4 - Solving a 1D, 3D or 3D Poisson Problem in Parallel</h1>
...
int main (int argc, char ** argv)
{
  ...
  libmesh_example_requires(dim <= LIBMESH_DIM, "3D/3D support");
  ...
  // Construct a convex hull for the hole
  //add_off_convex_hull_to_mesh(mesh, "innerEllipsoid.off");

#ifdef LIBMESH_HAVE_TETGEN
  ReplicatedMesh cube_mesh(init.comm(), 3);

  unsigned n_elem = 1;

  MeshTools::Generation::build_cube(cube_mesh,
                                    ps, ps, ps, // ps elements in each direction
                                    -1., 1.,
                                    -1., 1.,
                                    -1., 1.,
                                    HEX8);
  ...
  // of the convex hull.
  TetGenMeshInterface tc(cube_mesh);
  tc.pointset_convexhull();
  ...
  std::cout << *mesh.node_ptr(666) << std::endl;

  std::vector<dof_id_type> beids;
    ...
    if (old_elem->type() == TRI3)
      {
        ...
        beids.push_back(new_elem->id());
      }
#else
  // Avoid compiler warnings
  libmesh_ignore(mesh, -1., 1.);
  ...
  for (std::vector<dof_id_type>::iterator beid_it = beids.begin() ; beid_it != beids.end(); ++beid_it)
  {
    const Elem & belem = mesh.elem_ref(*beid_it);
    /*
    std::cout << "elem " << belem.id() << " " << belem.type() << std::endl;
    for (auto s : belem.side_index_range())
      mesh_bi.add_edge(&belem, s, 0);
    */
  }
  ...
  for (auto & elem : mesh.element_ptr_range())
  {
    ...
    for (auto s : elem->side_index_range())
      ...
      {
        borderline = 1;
        mesh_bi.add_side(elem, s, 0);
      }

    /*
    if ( borderline == 1 && elem->type() == TET4 )
      std::cout << "elem " << elem->id() << " " << elem->type() << std::endl;
    */
  }
